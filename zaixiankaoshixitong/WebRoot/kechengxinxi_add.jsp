...
function hsgcheck() {
		...
		if(kecheng==""||(kecheng.length<6||kecheng.length>12)){
			 $("#clabelkecheng").html("<font color=red>课程不能为空且长度在3～12位之间！</font>");
			 $("input[id=kecheng]").focus();
		...
		else
		{
			...
			$.ajax({
				...
				if(result.info=="ng"){
					$("#clabelkecheng").html("<font color=red>课程已存在，请更换！</font>");
					$("input[id=kecheng]").focus();
					$("#querenzhuce").attr("disabled","disabled");
					return false;
				}
